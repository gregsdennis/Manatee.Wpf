...
namespace Manatee.Ui.Mvvmc
{
	...
	public class EventAggregator : IEventAggregator
	{
		private class Handler
		{
			...
			private readonly Dictionary<TypeInfo, MethodInfo> _supportedHandlers = new Dictionary<TypeInfo, MethodInfo>();
			...
			public Handler(object handler)
			{
				...
				var interfaces = handler.GetType().GetTypeInfo().ImplementedInterfaces
				                        ...
				                        .Where(x => typeof(IHandle).GetTypeInfo().IsAssignableFrom(x) && x.IsGenericType);

				foreach (var @interface in interfaces)
				{
					var type = @interface.GenericTypeArguments[0];
					var method = @interface.DeclaredMethods.FirstOrDefault(m => m.Name == nameof(IHandle<object>.Handle) &&
					                                                            m.GetParameters()
					                                                             .Select(p => p.ParameterType)
					                                                             .SequenceEqual(new[] {type}));

					if (method != null)
						_supportedHandlers[type.GetTypeInfo()] = method;
				}
			}
			...
			public bool Handle(TypeInfo messageType, object message)
			{
			...
			public bool Handles(TypeInfo messageType)
			{
		...
		public bool HandlerExistsFor(Type messageType)
		{
			lock (_handlers)
			{
				return _handlers.Any(handler => handler.Handles(messageType.GetTypeInfo()) & !handler.IsDead);
			}
		}
		...
		public virtual void Publish(object message, Action<Action> marshal = null)
		{
			...
			marshal(() =>
				        {
					        var messageType = message.GetType().GetTypeInfo();
