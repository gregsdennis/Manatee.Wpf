...
{
	/// <summary>
	/// Combines values into a string with a specified format.
	/// </summary>
	public class StringFormatter : IMultiValueConverter
	{
		...
		/// </summary>
		public static StringFormatter Instance { get; }
		...
		{
			if (parameter != null && !(parameter is string)) throw new ArgumentException("If parameter is not null, it must be a string.");

			var format = (string) parameter;
			IEnumerable<object> args = values;
			if (format == null)
			{
				if (values.Length == 0) throw new ArgumentException("Values are required.");
